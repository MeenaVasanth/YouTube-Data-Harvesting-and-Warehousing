...
conn.autocommit = True  # Enable autocommit to allow dropping databases
cursor = conn.cursor()
...
# Initialize session state for channel IDs and data
if 'channel_ids' not in st.session_state:
    st.session_state.channel_ids = []
if 'channel_data' not in st.session_state:
    st.session_state.channel_data = {}  # Ensure this is a dictionary

# Input for channel IDs
st.markdown("<h5 style='text-align: left; color: black;'>Enter the Channel ID here</h5>", unsafe_allow_html=True)

# Text input for channel ID with a unique key
channel_id_input = st.text_input("Channel ID", "")  # Use a variable for input

# Save button for channel ID
if st.button("Save Channel ID"):
    if channel_id_input:
        if channel_id_input not in st.session_state.channel_ids:
            st.session_state.channel_ids.append(channel_id_input)
            st.success(f"Saved Channel ID: {channel_id_input}")
            channel_id_input = ""  # Clear the variable for next input (this does not affect the widget)
        else:
            st.warning("Channel ID already saved.")

# Display the list of saved channel IDs
if st.session_state.channel_ids:
    st.markdown("<h5 style='text-align: left; color: black;'>Saved Channel IDs:</h5>", unsafe_allow_html=True)
    for i, cid in enumerate(st.session_state.channel_ids, start=1):
        st.write(f"{i}: {cid}")

# Collect and store data for multiple channels
if st.button("Collect and Store Data"):
    for channel_id in st.session_state.channel_ids:
        # Assuming channel_Details is defined elsewhere and returns the necessary values
        output, c, p, v, com = channel_Details(channel_id)  
        st.code(output)
        
        # Store the data for each channel using the channel ID as the key
        st.session_state.channel_data[channel_id] = {
            'c': c,
            'p': p,
            'v': v,
            'com': com
    ...
    st.success("Channel data collected and stored.")
